class Dog:
    ...
    does this automatically for us but we have to have "self" as the 
    first parameter to the __init__ method (i.e. the constructor).
    '''
    ...
    object. Notice that "self" is a parameter. Every method has "self" as 
    its first parameter. The "self" parameter is a reference to the 
    current object. The current object appears on the left hand side of 
    the dot (i.e. the .) when the method is called.
    '''
    def speak(self):
    ...
    '''
    This is another accessor method that uses the birthday information
    to return a string representing the date.
    '''
    def birthDate(self):
        bd = str(self.month) + "/" + str(self.day) + "/" + str(self.year)
        return bd 
    ...
    def __add__(self, otherDog):
        return Dog("Puppy of " + self.name + " and " + otherDog.name, \
            self.month, self.day, self.year + 1, self.speakText + \
            otherDog.speakText)
